...
<div class="container-fluid1">
 ...
 <div class="wrapper">
        ...
        <div class=" main-header anim" style="--delay: .3s">All Tracks</div>
          @foreach ($tracks as $track )
              ...
              <div class="small-header anim" style="--delay: 0s;">{{$track->name}}</div>
                <div class="videos">
          
                 @foreach ($track->courses()->limit(5)->get() as $course )
                    <div class="video anim" style="--delay: .7s">
                       <div class="video-time">{{$course->created_at}}</div>
                        <div class="video-wrapper">
                            @if ($course->photo)
                            {{--False  <video muted="">
                            <source src="{{asset('/img/'.$course->photo->filename)}}" type="video/mp4">
                            </video>  --}}
                            <img src="{{asset('/img/'.$course->photo->filename)}}" style="height: auto;width: 100%;max-height: 132px;"> 
                            @else
                                <img src="https://images.hepsiburada.net/banners/s/0/672-378/bannerImage2121_20210311083848.png">
                            @endif
                            <a name="" id="" class="btn btn-light" href="/courses/{{$course->slug}}" role="button">go</a>
                        ...
                        <div class="video-by" style=@php if($course->status==1){echo 'color:green;';}else{echo 'color:white;';} @endphp>{{\Str::limit($course->title,100)}}
                          @php
                             if($course->status==1){
                                echo '<div class="spinner-grow text-success" role="status" style="width:0.6em;margin-top: 10px;height: 0.6em;margin-left: 6px;">
                                <span class="visually-hidden">Loading...</span>
                                </div>';
                                }else{
                                echo '<div class="spinner-grow text-light" role="status" style="height:0.6em;margin-top: 10px;width: 0.6em;margin-left: 6px;">
                                <span class="visually-hidden">Loading...</span>
                                </div>';
                                }
                          @endphp
                          </div>
                        <div class="video-name">{{$course->link}}</div>
                         {{--  <h3 class="{{($course->status==1)?'text-success':'text-muted'}}" style="text-align:center;">
                         {{($course->status==1)?strtoupper('Paid'):strtoupper('Free')}}
                         </h3>  --}}
                        <div class="video-view">{{count($course->users)}} subscribers<span class="seperate video-seperate"></span>{{$course->created_at}}</div>
                    </div>
                     @endforeach
